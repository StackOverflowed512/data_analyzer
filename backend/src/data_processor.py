...
class DataProcessor:
    ...
    def process_data_for_chart(self, chart_specs: Dict[str, Any]) -> Tuple[Optional[pd.DataFrame], Optional[str]]:
        ...
        try:
            ...
            # If we are aggregating, the columns are handled by the aggregation step.
            # BUT, if we have columns_only without aggregation, we filter.
            # AND if we have aggregation but it's "count", y_col is generated, so we shouldn't fail if y_col isn't in df.
            elif columns_only:
                # Filter out columns that don't exist in original DF, UNLESS they are generated by aggregation
                # If agg_method is 'count', y_col is generated, so we don't expect it in self.df
                
                cols_to_check = columns_only.copy()
                if agg_method == 'count' and y_col in cols_to_check:
                    cols_to_check.remove(y_col)
                
                valid_columns = [col for col in cols_to_check if col in self.columns]
                
                if not valid_columns and not (agg_method == 'count'):
                     # If we are counting, x_col is enough. If not counting and no valid cols, error.
                    return None, f"None of the requested columns {columns_only} exist."

                # If counting, ensuring x_col is present is critical
                if agg_method == 'count' and x_col and x_col not in valid_columns:
                     if x_col in self.columns:
                         valid_columns.append(x_col)
                
                # Ensure the sort_by column is included if specified
                if sort_by:
                    sort_cols = sort_by if isinstance(sort_by, list) else [sort_by]
                    for sc in sort_cols:
                         # If sorting by the generated count column, we don't need it in source df
                         if agg_method == 'count' and sc == y_col:
                             continue
                         if sc not in valid_columns and sc in self.columns:
                            valid_columns.append(sc)
                
                # Only apply column filtering if we are NOT aggregating
                # Aggregation creates a new DF structure anyway.
                if not agg_method:
    ...
    def _apply_aggregation(self, df: pd.DataFrame, x_col: str, y_col: str, agg_method: str) -> pd.DataFrame:
        ...
        try:
            if agg_method == "count":
                ...
                # groupby x_col, size() gives series with x_col as index and count as values
                # reset_index(name=y_col) converts it to dataframe with x_col and y_col (count)
                result = df.groupby(x_col).size().reset_index(name=y_col)
                # If y_col was already in df, it might cause issues if we don't handle it carefully,
                # but valid_columns check in caller should handle most cases.
                # However, if y_col doesn't conceptually exist for "count", we are creating it.
            else:
